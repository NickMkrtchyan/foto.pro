...
(function (factory) {
    ...
    var methods = {
        start: function (sec) {
            ...
            if (sec) {
                init.call(this, sec);
                intervalId = setTimeout(tick.bind(this), 1e3);
    ...
    $.fn.timeTo = function () {
        ...
        if (options.timeTo) {
            ...
            if (time > now) {
                options.seconds = Math.floor((time - now) / 1e3);
            } else {
                options.seconds = 0;
            }
            ...
            if (typeof time === "object" && time.getTime) {
                options.seconds = time.getDate() * SECONDS_PER_DAY + time.getHours() * SECONDS_PER_HOUR + time.getMinutes() * 60 + time.getSeconds();
        ...
        return this.each(function () {
            var $this = $(this);
            var data = $this.data();
            var defs, opt, i, css, language, left, ulhtml, style, dhtml1, dhtml2, dot2, maxWidth, captionSize, fsStyleVal, fsStyle, thtml, marginRight, dhtml, $digits, dif, vals, limits;
            if (data.intervalId) {
            ...
            if (!data.vals) {
                ...
                css = { fontFamily: data.fontFamily };
                if (data.fontSize > 0) {
                ...
                thtml = (data.displayCaptions
                    ? (data.displayHours ? '<figure style="max-width:' + maxWidth + 'px">$1<figcaption' + fsStyle + ">" + language.hours + "</figcaption></figure>" + dot2 : "") +
                ...
                if (data.displayDays > 0) {
                    ...
                    for (i = data.displayDays - 1; i > 0; i -= 1) {
                        dhtml += i === 1 ? dhtml2.replace('">', "margin-right:" + 11 + 'px">') : dhtml2;
                    }
                    ...
                    thtml =
                        (data.displayCaptions
                            ? '<figure style="width:' + (data.width * data.displayDays + marginRight + 4) + 'px">$1' + '<figcaption style="' + fsStyleVal + "padding-right:" + marginRight + 'px">' + language.days + "</figcaption></figure>"
                            : "$1"
    ...
    function init(sec, force) {
        ...
        rest += minutes * 60;
        secs = sec - rest;
        str = (days < 100 ? "0" + (days < 10 ? "0" : "") : "") + days + (hours < 10 ? "0" : "") + hours + (minutes < 10 ? "0" : "") + minutes + (secs < 10 ? "0" : "") + secs;
        ...
        if (isInterval || force) {
            ...
            data.intervalId = setTimeout(tick.bind(this), 1e3);
    ...
    function tick(digit) {
        ...
        } else if (!data.countdown && digit === data.iHour && data.displayDays > 0 && data.vals[digit - 1] === 2 && data.vals[digit] === 3) {
            n = 0;
            tick.call(this, digit - 1);
        }
        $li.eq(0).html(n);
